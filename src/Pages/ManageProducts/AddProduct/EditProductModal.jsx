...
const EditProductModal = ({ isOpen, onClose, product, groups = [], onUpdateProduct }) => {
  ...
  useEffect(() => {
    if (product) {
      ...
      const selectedGroup = product.productGroup || product.group || product.groupName || "";
      const selectedSubGroup = product.productSubGroup || product.subGroup || product.subGroupName || "";
      ...
      setFormData({
        ...
        unitPrice: product.unitPrice ?? product.tpPrice ?? product.price ?? 0,
  ...
  const updateSubGroupsList = (groupName, groupsList) => {
    ...
    if (foundGroupObj) {
      const subList = foundGroupObj.subGroups || foundGroupObj.subgroups || foundGroupObj.subGroup || [];
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: ["quantity", "unitPrice", "purchasePrice", "weight"].includes(name)
        ? Number(value)
        : value,
    }));
  };
  ...
  const handleSubmit = (e) => {
    ...
    Swal.fire({
      ...
      customClass: {
        ...
        title: "text-lg font-bold text-gray-800 text-left border-b border-gray-200 pb-3 mb-3",
        htmlContainer: "text-gray-600 text-sm text-left my-3 font-normal",
        actions: "flex justify-end gap-2 border-t border-gray-200 pt-3 mt-3 w-full",
        confirmButton: "bg-teal-600 hover:bg-teal-700 text-white font-medium px-4 py-2 rounded border-0 text-sm",
        cancelButton: "bg-white hover:bg-gray-100 text-gray-700 font-medium px-4 py-2 rounded border border-gray-300 text-sm"
      },
  ...
  const totalWeight = (formData.quantity || 0) * (formData.weight || 0);
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden border border-gray-100">
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-gray-50 border-b border-gray-200">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-gray-800">Edit Product Details</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="bg-gray-50/50 p-4 rounded-xl border border-gray-200 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {/* Group Select */}
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Product Group</label>
                <select
                  name="productGroup"
                ...
                >
                  ...
                  {groups && groups.length > 0 ? (
                    groups.map((group, idx) => {
                      const gName = group.groupName || group.name || group.group || group;
                      return (
                    ...
                    formData.productGroup && (
                      <option value={formData.productGroup}>{formData.productGroup}</option>
                    )
                  )}
                </select>
              </div>
              ...
              {/* Sub Group Select */}
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Product Sub Group</label>
                <select
                ...
                >
                  ...
                  {availableSubGroups && availableSubGroups.length > 0 ? (
                    availableSubGroups.map((subGroup, idx) => {
                      const subName = typeof subGroup === 'string' ? subGroup : (subGroup.name || subGroup.subGroupName || subGroup.title);
                      return (
                    ...
                    formData.productSubGroup && (
                      <option value={formData.productSubGroup}>{formData.productSubGroup}</option>
                    )
                  )}
                </select>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="p-5 border border-gray-200 rounded-xl space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Wt / Unit (kg)</label>
                  <input
                    type="number"
                    name="weight"
                    step="0.01"
            ...
            {/* Summary */}
            <div className="p-5 border border-gray-200 bg-gray-50/40 rounded-xl flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <Calculator className="size-4 text-gray-500" />
                  <h3 className="text-xs font-bold text-gray-500 tracking-wider uppercase">Summary Calculation</h3>
                </div>

                <div className="space-y-2 text-sm text-gray-600 border-b border-gray-200 pb-3">
                  <div className="flex justify-between">
                    <span>Unit Price:</span>
                    <span className="font-semibold text-gray-800">৳ {Number(formData.unitPrice || 0).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between">
